...
@router.get("/tools")
def tools(identity: AuthIdentity = Depends(get_auth_identity), repo_root: Path = Depends(get_repo_root)) -> dict[str, Any]:
    return {"success": True, "result": {"tools": MCPService(repo_root).discover_tools()}, "metadata": {"auth_enabled": identity.auth_enabled}}
