...
Note: the local `legacy_mcp/` directory was renamed from `mcp/` specifically to stop
shadowing the real `mcp` PyPI package when `PYTHONPATH=.` is set — see the workaround
comment at the top of `legacy_mcp/server.py` for the original `ModuleNotFoundError: No
module named 'mcp.types'` issue this fixed.
