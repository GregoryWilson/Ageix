...
MCP_TOOL_DEFINITIONS: tuple[MCPToolDefinition, ...] = (
    ...
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            "status": _string("Optional DevJob status filter.", enum=["draft", "assigned", "in_progress", "submitted", "reviewed", "completed", "blocked", "declined", "cancelled"]),
    ...
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            ...
            "work_context_id": _string("Resolvable WORKCTX package ID; assignment is denied without one."),
            "acceptance_criteria": _array("List of criteria the result must satisfy."),
            "allowed_paths": _array("Repository paths the assignee may touch."),
            "prohibited_paths": _array("Repository paths the assignee must not touch."),
            "instructions": _array("Ordered list of instructions for the assignee."),
            "assigned_to": _string("Identity of the assignee."),
            "actor_id": _string("Explicit actor identity; defaults to the caller's client_id."),
            "agent_role": _string("Caller's self-declared agent role."),
            "note": _string("Optional note recorded with the assignment transition."),
        }, ["job_id"]),
        recommended_next_tools=("ageix.devjob.get", "ageix.devjob.transition"),
        related_tools=("ageix.devjob.create", "ageix.architecture.work_context.get"),
        documentation={
            "use_when": ["Move a draft DevJob into assigned status once a Work Context, scope, and assignee are known."],
            "do_not_use_when": ["Do not use this on a DevJob that is not currently in draft status."],
            "authority": "The DevJob's creator, Greg, or an authorized governance role.",
            "intent_tags": ["devjob", "assignment", "coordination_contract"],
        },
    ),
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            ...
            "target_status": _string("Target lifecycle status.", enum=["in_progress", "submitted", "blocked", "declined", "reviewed", "completed", "cancelled"]),
            "actor_id": _string("Explicit actor identity; defaults to the caller's client_id."),
            "agent_role": _string("Caller's self-declared agent role."),
            "note": _string("Reason for the transition; required for blocked and declined targets."),
        }, ["job_id", "target_status"]),
        recommended_next_tools=("ageix.devjob.get", "ageix.devjob.event.list"),
        related_tools=("ageix.devjob.review.submit", "ageix.devjob.sync.attach", "ageix.devjob.validation.waiver"),
        documentation={
            "use_when": ["Advance or halt a DevJob's lifecycle status directly, e.g. starting work, blocking, or cancelling."],
            "do_not_use_when": ["Do not use this for draft-to-assigned (use devjob.assign) or for review decisions on a submission (use devjob.review.submit)."],
            "authority": "Varies by target status: the assigned DevWorker, a reviewer, Greg, or governance, per INTENT-0007 lifecycle authority rules.",
            "intent_tags": ["devjob", "lifecycle", "coordination_contract"],
        },
    ),
    MCPToolDefinition(
        ...
        recommended_next_tools=("ageix.devjob.get",),
        documentation={
            "use_when": ["Inspect the complete governed history of a DevJob, including scope revisions, waivers, sync references, and review decisions, not just status changes."],
            "do_not_use_when": ["Do not use this for a quick status check — use devjob.get instead."],
            "authority": "Governed read-style capability.",
            "intent_tags": ["devjob", "event_history", "coordination_contract"],
        },
    ),
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            "job_id": _string("DevJob ID to review."),
            "decision": _string("Review decision.", enum=["approved", "changes_requested"]),
            "reviewer_notes": _string("Reviewer's notes; required as the decline reason when decision is changes_requested."),
            "actor_id": _string("Explicit actor identity; defaults to the caller's client_id."),
            "agent_role": _string("Caller's self-declared agent role."),
        }, ["job_id", "decision"]),
        recommended_next_tools=("ageix.devjob.get", "ageix.devjob.transition"),
        related_tools=("ageix.devjob.event.list",),
        documentation={
            "use_when": ["Record a reviewer's (or Greg's) decision on a submitted DevJob result."],
            "do_not_use_when": ["Do not use this to move a DevJob from reviewed to completed — use devjob.transition for that."],
            "authority": "A reviewer role (Lex) or Greg.",
            "intent_tags": ["devjob", "review", "coordination_contract"],
        },
    ),
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            ...
            "note": _string("Optional note about the sync reference."),
            "actor_id": _string("Explicit actor identity; defaults to the caller's client_id."),
            "agent_role": _string("Caller's self-declared agent role."),
        }, ["job_id"]),
        recommended_next_tools=("ageix.devjob.get", "ageix.devjob.transition"),
        documentation={
            "use_when": ["Record where a DevJob's work landed (branch/PR/commit) so completion's git-synchronization requirement can be satisfied."],
            "do_not_use_when": ["Do not use this to perform any git operation — it only stores a pointer reported by the caller."],
            "authority": "The DevJob's assigned DevWorker, Greg, or an authorized governance role.",
            "intent_tags": ["devjob", "git_sync", "coordination_contract"],
        },
    ),
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            "job_id": _string("DevJob ID whose scope is being revised."),
            "reason": _string("Reason for the scope revision."),
            "evidence_package_ids": _array("Evidence package IDs supporting this revision; required and non-empty."),
            "instructions": _array("Revised ordered list of instructions, if changed."),
            "acceptance_criteria": _array("Revised acceptance criteria, if changed."),
            "allowed_paths": _array("Revised allowed paths, if changed."),
            "prohibited_paths": _array("Revised prohibited paths, if changed."),
            "actor_id": _string("Explicit actor identity; defaults to the caller's client_id."),
            "agent_role": _string("Caller's self-declared agent role."),
        }, ["job_id", "reason", "evidence_package_ids"]),
        recommended_next_tools=("ageix.devjob.get", "ageix.devjob.event.list"),
        related_tools=("ageix.evidence_package.details",),
        documentation={
            "use_when": ["Change a DevJob's scope (instructions, acceptance criteria, or paths) while it is assigned, in_progress, or blocked, with supporting evidence."],
            "do_not_use_when": ["Do not use this to change a DevJob's core identity fields (title, objective, creator) — those are immutable after creation."],
            "authority": "The DevJob's creator, Greg, or an authorized governance role.",
            "intent_tags": ["devjob", "scope_revision", "coordination_contract"],
        },
    ),
    MCPToolDefinition(
        ...
        input_schema=_object_schema({
            "job_id": _string("DevJob ID to record the waiver against."),
            "reason": _string("Reason validation is being waived."),
            "actor_id": _string("Explicit actor identity; defaults to the caller's client_id."),
            "agent_role": _string("Caller's self-declared agent role."),
        }, ["job_id", "reason"]),
        recommended_next_tools=("ageix.devjob.get", "ageix.devjob.transition"),
        documentation={
            "use_when": ["Document that a DevJob is completing without an attached validation run, per a deliberate governance decision."],
            "do_not_use_when": ["Do not use this to skip or replace running actual validation when validation is available — it is a documented exception, not a substitute."],
            "authority": "Greg or an authorized governance role only.",
            "intent_tags": ["devjob", "validation_waiver", "coordination_contract"],
        },
    ),
