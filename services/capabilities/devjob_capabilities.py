...
def register_capabilities(repo_root: Path):
    ...
    def devjob_transition(arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            job = registry().transition_job(
                job_id, target_status,  # type: ignore[arg-type]
                actor_id=actor_id, actor_role=actor_role, note=str(arguments.get("note") or ""),
            )
        except ValueError as exc:
            return {"success": False, "result": {}, "error": str(exc)}
        return {"success": True, "result": job.to_metadata(), "metadata": {"source": "devjob_registry_service"}}
    ...
    def devjob_event_list(arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "result": {}, "error": str(exc)}
        return {"success": True, "result": {"events": result}, "metadata": {"source": "devjob_registry_service"}}
    ...
    def devjob_review_submit(arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            job = registry().submit_review(
                job_id, decision=str(arguments.get("decision") or ""),
    ...
    def devjob_sync_attach(arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            job = registry().attach_sync(
                job_id, branch=arguments.get("branch"), pr_reference=arguments.get("pr_reference"),
                commit_sha=arguments.get("commit_sha"), note=str(arguments.get("note") or ""),
                actor_id=actor_id, actor_role=actor_role,
            )
        except ValueError as exc:
            return {"success": False, "result": {}, "error": str(exc)}
    ...
    def devjob_scope_revise(arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            job = registry().revise_scope(
                job_id, reason=str(arguments.get("reason") or ""),
                evidence_package_ids=arguments.get("evidence_package_ids") or [],
                instructions=arguments.get("instructions"),
                acceptance_criteria=arguments.get("acceptance_criteria"),
                allowed_paths=arguments.get("allowed_paths"),
    ...
    return [
        ...
        (CapabilityDefinition(
            ...
            description="Move a draft DevJob to assigned, requiring acceptance criteria, allowed/prohibited paths, an assigned worker, and an authorized assigner. A Work Context is optional at assignment and required only at the DevWorker execution boundary, per INTENT-0007 Phase 2.",
        ), devjob_assign),
        (CapabilityDefinition(
