...
from models.evidence_package import EvidencePackage, EvidencePackageItem
...
class EvidenceBrokerService:
    ...
    def request_evidence(
        ...
        *,
        ...
        candidates = self._candidate_paths(plan)
    ...
    def _candidate_paths(self, plan: EvidencePlan) -> list[str]:
        ...
        for path in [*hinted, *discovered]:
            if path not in ordered and self._retrievable(path):
                ordered.append(path)
    ...
    def _retrievable(self, path: str) -> bool:
        relative = Path(path)
        if relative.is_absolute() or ".." in relative.parts:
            return False
        lower = path.lower()
        if lower.startswith(".ageix/") or lower == ".ageix":
            return False
        if set(Path(lower).parts) & self.GENERATED_PARTS:
            return False
        if any(lower.endswith(suffix) for suffix in self.GENERATED_SUFFIXES):
            return False
        full = self.repo_root / path
        return full.is_file() and full.suffix.lower() in self.TEXT_SUFFIXES
    ...
    def _build_item(self, path: str, plan: EvidencePlan, hinted: bool) -> EvidencePackageItem:
        ...
        return EvidencePackageItem(
            ...
            metadata={"full_file_returned": not excerpted, "strong_relevance": strong_relevance},
        )
