...
class AgentSessionService:
    ...
    def save_session(self, session: AgentSession) -> AgentSession:
        data = self._load()
        data.setdefault("sessions", {})[session.session_id] = session.model_dump()
        data["updated_at"] = datetime.now(timezone.utc).isoformat()
        self._write(data)
        return session
    ...
    def record_capability_use(
        ...
        session = self.get_session(session_id) or self.create_session(session_id, agent_id, project_id=project_id, metadata=metadata)
