...
def validate_assignment_fields(job: DevJob) -> None:
    """Raises ValueError if a deliberate execution-ready assignment lacks the
    scope fields INTENT-0007 Phase 2 requires.

    NOTE (Sprint 21.x work-context relocation): work_context_id is intentionally
    NOT required here. A DevJob may be created or assigned without a Work
    Context; the Work Context is required only at the DevWorker execution /
    load-context boundary, where execution genuinely needs it. This validator is
    used only by the deliberate `assign_job` path, never by generic DevJob
    creation, so it does not break generic assignment, admission, launcher, CLI,
