...
class ProposalApprovalService:
    """Target-specific governed proposal approval execution.

    Human Interface callers may route here, but all proposal lifecycle mutation is
    performed by this target-specific capability family.
    """
    ...
    TERMINAL_STATUSES = {
        ...
        ProposalStatus.APPROVED_WITH_CONDITIONS,
    ...
    def execute(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            proposal = self.proposals.get_proposal(clean["target_record_id"])
        except FileNotFoundError:
            return self._failure("target_not_found", clean)

        if proposal.project_id != self.REQUIRED_PROJECT_ID:
            return self._failure("project_scope_denied", clean, status_before=proposal.status.value)

        status_before = proposal.status.value
        action = clean["action"]

        if action == "approve":
            return self._approve(clean, proposal, status_before)
        if action == "reject":
            return self._reject(clean, proposal, status_before)
        if action == "add_comment":
            return self._add_comment(clean, proposal, status_before)
    ...
    def _approve(self, clean: dict[str, Any], proposal: Proposal, status_before: str) -> dict[str, Any]:
        if proposal.status in self.TERMINAL_STATUSES:
        ...
        if proposal.status not in self.MUTABLE_STATUSES:
            return self._failure("invalid_target_state", clean, status_before=status_before, status_after=status_before)
        metadata = self._metadata_with_governance_event(proposal.metadata, clean, status_before, ProposalStatus.APPROVED.value)
        updated = self.proposals.update_status(proposal.proposal_id, ProposalStatus.APPROVED, metadata=metadata)
        return self._success(clean, status_before, updated.status.value)

    def _reject(self, clean: dict[str, Any], proposal: Proposal, status_before: str) -> dict[str, Any]:
        if proposal.status in self.TERMINAL_STATUSES:
            return self._failure("invalid_target_state", clean, status_before=status_before, status_after=status_before)
        if proposal.status not in self.MUTABLE_STATUSES:
            return self._failure("invalid_target_state", clean, status_before=status_before, status_after=status_before)
        metadata = self._metadata_with_governance_event(proposal.metadata, clean, status_before, ProposalStatus.DENIED.value)
        updated = self.proposals.update_status(proposal.proposal_id, ProposalStatus.DENIED, metadata=metadata)
        return self._success(clean, status_before, updated.status.value)

    def _add_comment(self, clean: dict[str, Any], proposal: Proposal, status_before: str) -> dict[str, Any]:
        metadata = self._metadata_with_governance_event(proposal.metadata, clean, status_before, status_before)
        updated = self.proposals.update_status(proposal.proposal_id, proposal.status, metadata=metadata)
    ...
    def _success(
        ...
        *,
        ...
        return {
            ...
            "metadata": {
                "source": "proposal_approval_service",
                "target_domain": "proposal",
            },
        }
